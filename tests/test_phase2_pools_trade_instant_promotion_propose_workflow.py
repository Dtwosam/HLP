...
WORKFLOW = Path(
    ".github/workflows/phase2-pools-trade-instant-promotion-propose.yml"
)


def test_pools_trade_instant_proposal_is_manual_and_proposal_only():
...
def test_pools_trade_instant_proposal_requires_exact_review_handoff():
    text = WORKFLOW.read_text()
    assert "phase2-pools-trade-instant-promotion-review.yml" in text
...
def test_pools_trade_instant_proposal_uses_duplicate_safe_dispatcher():
    text = WORKFLOW.read_text()
    assert "phase2-execution-node-dispatch.yml" in text
    assert '"node_id": "promote:pools_trade_instant"' in text
    assert "phase2-execution-node-dispatch-attempt.json" in text
    assert "validate_phase2_node_dispatch_evidence" in text


def test_pools_trade_instant_proposal_validates_single_source_advance():
    text = WORKFLOW.read_text()
    assert 'expected_source_id="pools_trade_instant"' in text
    assert "validate_phase2_coverage_ledger_commit" in text
    assert "phase2-source-coverage-promotion-pools_trade_instant" in text
    assert '"proposal_validated": True' in text


def test_pools_trade_instant_proposal_never_authorizes_ledger_write():
    text = WORKFLOW.read_text()
