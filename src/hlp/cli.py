        ...
        json.dumps(
            {
                ...
                "request_bytes_sent": rpc.request_bytes_sent,
...
def cmd_phase2_pools_trade_instant_market_window(
    args: argparse.Namespace,
) -> int:
    """Reconstruct one bounded pools.trade Instant V4 market-cap window."""
    if args.from_block <= 0:
        raise SystemExit("from-block must be > 0")
    if args.to_block < args.from_block:
        raise SystemExit("to-block must be >= from-block")
    ...
    registry = _load_jsonl(args.registry)
    if not registry:
        raise SystemExit("pools.trade Instant registry is empty")
    if {
        str(row.get("source_id") or "")
        for row in registry
    } != {"pools_trade_instant"}:
        raise SystemExit("pools.trade Instant registry source identity changed")
    ...
    for row in registry:
        ...
        token = normalize_address(str(row["token"]))
        if pool_id in by_pool:
            raise SystemExit(f"pools.trade Instant registry repeats pool: {pool_id}")
        if token in tokens:
            raise SystemExit(f"pools.trade Instant registry repeats token: {token}")
        by_pool[pool_id] = row
        tokens.add(token)
    ...
    for raw in _iter_jsonl(args.swaps):
        ...
        if not args.from_block <= block <= args.to_block:
            raise SystemExit(
                f"pools.trade Instant swap outside requested window: {block}"
            )
        swaps.append(row)
    swaps.sort(key=event_order)
    ...
    for raw in _iter_jsonl(args.supply_deltas):
        ...
        if token not in tokens:
            raise SystemExit(
                f"pools.trade Instant supply tape contains unknown token: {token}"
            )
        if int(row["block_number"]) <= args.to_block:
            supply_deltas.append(row)
    supply_deltas.sort(
    ...
    if missing_decimals:
        raise SystemExit(
            "pools.trade Instant quote allowlist is missing: "
            + ", ".join(missing_decimals)
        )

    point_provenance = {
        "source": "phase2_pools_trade_instant_v4_market_window",
        "chain_id": 4663,
    ...
    if not target_events:
        ...
        summary_manifest = write_jsonl_snapshot(
            ...
            output=Path(args.summary_out),
            provenance={
                "source": "phase2_pools_trade_instant_v4_market_window_summary",
                "market_cap_points_sha256": points_manifest["sha256"],
                "from_block": args.from_block,
                "to_block": args.to_block,
            },
        ...
        report = {
            "version": "phase2-pools_trade_instant-market-window-v1",
            "source_id": "pools_trade_instant",
            "from_block": args.from_block,
            "to_block": args.to_block,
            "initialize_events": 0,
    ...
    if missing_feeds:
        raise SystemExit(
            "pools.trade Instant quote feed registry is missing: "
            + ", ".join(missing_feeds)
        )
    sparse_chainlink_points = build_sparse_chainlink_usd_points(
        rpc,
...
def cmd_phase2_pools_trade_lbp_migrated_registry(
    args: argparse.Namespace,
) -> int:
    """Join LBP registry to the complete shared V4 Initialize surface."""
    registry = _load_jsonl(args.registry)
    initializes = _load_jsonl(args.initializes)
    if not registry:
        raise SystemExit("pools.trade LBP registry is empty")

    migrated = attach_pools_trade_lbp_initializations(
        registry,
        initializes,
    )
    manifest = write_jsonl_snapshot(
        ...
        output=Path(args.out),
        provenance={
            "source": "phase2_pools_trade_lbp_migrated_registry",
            "chain_id": 4663,
            "registry": Path(args.registry).name,
            "registry_sha256": _sha256_file(args.registry),
            "initializes": Path(args.initializes).name,
            "initializes_sha256": _sha256_file(args.initializes),
            "migration_identity": "exact derived PoolId plus full PoolKey",
        },
    )
    delays = sorted(
        int(row["migration_delay_blocks"])
        for row in migrated
    )
    summary = {
        "version": "phase2-pools-trade-lbp-migrated-registry-v1",
        "source_id": "pools_trade_lbp",
        "registry_tokens": len(registry),
        "migrated_tokens": len(migrated),
        "non_migrated_tokens": len(registry) - len(migrated),
        "migrated_registry_sha256": manifest["sha256"],
        "base_registry_sha256": _sha256_file(args.registry),
        "v4_initialize_sha256": _sha256_file(args.initializes),
...
def cmd_phase2_pools_trade_lbp_v4_market_window(
    args: argparse.Namespace,
) -> int:
    """Reconstruct one bounded migrated-LBP V4 market-cap window."""
    if args.from_block <= 0:
        raise SystemExit("from-block must be > 0")
    if args.to_block < args.from_block:
        raise SystemExit("to-block must be >= from-block")
    ...
    if not registry:
        points_manifest = write_jsonl_snapshot(
            ...
            output=Path(args.out),
            provenance={
                "source": "phase2_pools_trade_lbp_v4_market_window",
                "chain_id": 4663,
                "source_id": "pools_trade_lbp",
                "registry": Path(args.registry).name,
                "registry_sha256": _sha256_file(args.registry),
        ...
        summary_manifest = write_jsonl_snapshot(
            ...
            output=Path(args.summary_out),
            provenance={
                "source": "phase2_pools_trade_lbp_v4_market_window_summary",
                "market_cap_points_sha256": points_manifest["sha256"],
                "from_block": args.from_block,
                "to_block": args.to_block,
            },
        ...
        report = {
            "version": "phase2-pools-trade-lbp-v4-market-window-v1",
            "source_id": "pools_trade_lbp",
            "phase": "v4",
            "from_block": args.from_block,
            "to_block": args.to_block,
    ...
    for raw in registry:
        row = dict(raw)
        if str(row.get("source_id") or "") != "pools_trade_lbp":
            raise SystemExit("pools.trade LBP migrated registry source changed")
        pool_id = str(row["pool_id"]).lower()
        token = normalize_address(str(row["token"]))
        if pool_id in by_pool:
            raise SystemExit(
                f"pools.trade LBP migrated registry repeats pool: {pool_id}"
            )
        if token in migrated_tokens:
            raise SystemExit(
                f"pools.trade LBP migrated registry repeats token: {token}"
            )
        by_pool[pool_id] = row
        migrated_tokens.add(token)
    ...
    for raw in _iter_jsonl(args.swaps):
        ...
        if not args.from_block <= block <= args.to_block:
            raise SystemExit(
                f"pools.trade LBP V4 swap outside requested window: {block}"
            )
        swaps.append(row)
    swaps.sort(key=event_order)
    ...
    for raw in _iter_jsonl(args.supply_deltas):
        row = dict(raw)
        token = normalize_address(str(row["token"]))
        if token not in migrated_tokens:
            continue
        if int(row["block_number"]) <= args.to_block:
            row["token"] = token
            supply_deltas.append(row)
    ...
    if missing_decimals:
        raise SystemExit(
            "pools.trade LBP V4 quote allowlist is missing: "
            + ", ".join(missing_decimals)
        )
    ...
    point_provenance = {
        ...
        "market_cap_math": (
            "raw_quote_per_raw_token * causal supply_raw / "
            "10**quote_decimals"
        ),
    }
    ...
    sparse_targets = [
        {
            **event,
            "quote_token": quote_by_pool[
                str(event["pool_id"]).lower()
            ],
        }
        for event in target_events
    ]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pools_trade_instant_market = sub.add_parser(
        "phase2-pools-trade-instant-market-window"
    )
    pools_trade_instant_market.add_argument("--registry", required=True)
    pools_trade_instant_market.add_argument("--swaps", required=True)
    pools_trade_instant_market.add_argument("--supply-deltas", required=True)
    pools_trade_instant_market.add_argument(
        "--from-block", type=int, required=True
    )
    pools_trade_instant_market.add_argument(
        "--to-block", type=int, required=True
    ...
    pools_trade_instant_market.add_argument("--quote-decimals", required=True)
    pools_trade_instant_market.add_argument("--quote-feeds", required=True)
    pools_trade_instant_market.add_argument("--out", required=True)
    pools_trade_instant_market.add_argument("--summary-out", required=True)
    ...
    pools_trade_lbp_migrated = sub.add_parser(
        "phase2-pools-trade-lbp-migrated-registry"
    )
    pools_trade_lbp_migrated.add_argument("--registry", required=True)
    pools_trade_lbp_migrated.add_argument("--initializes", required=True)
    pools_trade_lbp_migrated.add_argument("--out", required=True)
    pools_trade_lbp_migrated.add_argument(
        "--summary-out", required=True
    )
    pools_trade_lbp_migrated.set_defaults(
        func=cmd_phase2_pools_trade_lbp_migrated_registry
    )

    pools_trade_lbp_v4_market = sub.add_parser(
        "phase2-pools-trade-lbp-v4-market-window"
    )
    pools_trade_lbp_v4_market.add_argument("--registry", required=True)
    pools_trade_lbp_v4_market.add_argument("--swaps", required=True)
    pools_trade_lbp_v4_market.add_argument(
    ...
    pools_trade_lbp_v4_market.add_argument("--out", required=True)
    pools_trade_lbp_v4_market.add_argument(
        "--summary-out", required=True
    )
    pools_trade_lbp_v4_market.add_argument(
    ...
    pools_trade_lbp_v4_market.set_defaults(
        func=cmd_phase2_pools_trade_lbp_v4_market_window
    )
